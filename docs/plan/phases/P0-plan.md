...
1. **443 端口权限（macOS/Linux）**
   - 风险：直接监听 443 需要管理员权限，可能导致启动失败
   - 缓解：按 ADR-0002 选择方案 A：默认监听 443；启动失败时给出明确提示；必要时允许切换到高位端口用于开发/诊断
   - 状态：✅ 已实现默认监听 443（启动时提示权限不足）
...
8. **目标服务器证书验证跳过**
   ...
   - 缓解：默认行为对齐 ghosxy（可连自签名/中转）；后续在 P3/P5 增加“可配置开关 + 风险提示”
   - 状态：⚠️ 待完善（不阻塞主链路）

### P0 PoC 限制

- （已移除）固定转发目标：现已接入规则与 Provider 选路
- 端口转发自动配置：当前按 ADR-0002 选择方案 A，不做端口转发
- 日志持久化与更完整的统一体验：留给 P4/P5 完善
