...
- 独立的 **抓包** 页：打开录制开关后，**全量不脱敏**记录转发的出站
  URL、请求头、请求体与上游响应（响应头 + 响应正文，含流式）；左侧栏按
  会话保留，可随时回看、删除
- **安全提示**：全量模式会把明文 API Key 与完整对话/响应写入本地数据库，
  导出文件切勿分享。删除单个会话为逻辑删除，"清空全部"会执行 VACUUM
  真正回收磁盘
- 录制中或已结束的会话都可 **导出** 为 JSON（可按数据类别勾选：URL/请求头/
  请求体/响应头/响应体），弹系统保存对话框
- 明细页对超大内容只显示预览（前 256KB），完整内容请导出查看
- 抓包数据总量达到 200MB 时页面顶部提醒清理
- 录制开关随应用重启自动停止（未正常关闭的会话标记"已中断"），
  会话数据持久保留
