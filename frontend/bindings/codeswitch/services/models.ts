...
export class EndpointRecord {
    ...
    /**
     * 最后一次测速时间（Unix 时间戳），nil 表示未测试
     */
    "lastTestTime": number | null;

    /**
     * 最后一次测试速度（毫秒），nil 表示失败或未测试
     */
    "lastTestSpeed": number | null;

    /** Creates a new EndpointRecord instance. */
    constructor($$source: Partial<EndpointRecord> = {}) {
        if (!("url" in $$source)) {
            this["url"] = "";
        }
        ...
        if (!("lastTestSpeed" in $$source)) {
            this["lastTestSpeed"] = null;
        }
    ...
    /**
     * Creates a new EndpointRecord instance from a string or object.
     */
    static createFrom($$source: any = {}): EndpointRecord {
        let $$parsedSource = typeof $$source === 'string' ? JSON.parse($$source) : $$source;
        return new EndpointRecord($$parsedSource as Partial<EndpointRecord>);
    }
}
